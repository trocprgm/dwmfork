...
void manage(Window w, XWindowAttributes *wa) {
  ...
  updatesizehints(c);
  updatewmhints(c);
  // if (c->isfloating && (c->x == c->mon->mx || c->x == 0) && (c->y ==
  // c->mon->my || c->y == 0)) {
  if (c->isfloating && (c->x == c->mon->mx || c->x == 0)) {
    // vvv Always center patch
...
unsigned int nexttag(void) {
  ...
  /* keep only first selected tag */
  if (seltag & (seltag - 1))
    seltag = seltag & -seltag;
